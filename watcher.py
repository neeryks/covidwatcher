...
class dataframes():
    ...
    new = pd.concat([covidstat(),vaccstat()],ignore_index=True)
    new = new[['date','total_cases','people_vaccinated']].fillna(0)
    new.to_csv('data/new.csv')

sns.relplot(x='date',y='people_vaccinated',data = dataframes.new,kind='line')
plt.show()
